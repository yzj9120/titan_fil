...
AppId={{1B439C35-65C3-4569-8569-319BD540814B}
AppName={#MyAppName}
...
// 记录查询上一次的历史安装路径
var
OldInstallPathGlobal: string;
function InitializeSetup(): Boolean;
var
RegKey32, RegKey64: string;
begin
Result := True;
RegKey32 := 'SOFTWARE\WOW6432Node\Microsoft\Windows\CurrentVersion\Uninstall\{E74359E7-7418-42A4-8A3E-D3DF1D5276B8}_is1';
RegKey64 := 'SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\{E74359E7-7418-42A4-8A3E-D3DF1D5276B8}_is1';
if not RegQueryStringValue(HKLM, RegKey64, 'InstallLocation', OldInstallPathGlobal) then
RegQueryStringValue(HKLM, RegKey32, 'InstallLocation', OldInstallPathGlobal);
end;
...
begin
// {userappdata} 会自动解析为：C:\Users\<当前用户名>\AppData\Roaming
Result := ExpandConstant('{userappdata}\com.titan_fil\titan_fil\titan_fil_agent');
end;
...
ResultCode: Integer;
VirtualBoxPreInstalled: Boolean;
MultipassPreInstalled: Boolean;
...
var
VirtualBoxNowInstalled: Boolean;
MultipassNowInstalled: Boolean;
NeedShowRestartPrompt: Boolean;
begin
if CurStep = ssInstall then
begin
// 记录安装前状态
VirtualBoxPreInstalled := IsVirtualBoxInstalled;
MultipassPreInstalled := IsMultipassInstalled;
end;

if CurStep = ssPostInstall then
begin
SaveStringToFile(ExpandConstant('{app}\userlanguage.txt'), ActiveLanguage, False);
if OldInstallPathGlobal <> '' then
begin
SaveStringToFile(ExpandConstant('{app}\last_install_path.txt'), OldInstallPathGlobal, False);
end;
end;
...
begin
// 安装后再次检查
VirtualBoxNowInstalled := IsVirtualBoxInstalled;
MultipassNowInstalled := IsMultipassInstalled;

// 判断是否需要弹出重启提示：
// 只要 VirtualBox 或 Multipass 有一个是“新安装”（即安装前未装，安装后已装），就弹
NeedShowRestartPrompt :=
(not VirtualBoxPreInstalled and VirtualBoxNowInstalled) or
(not MultipassPreInstalled and MultipassNowInstalled);

if NeedShowRestartPrompt then
begin
// 保留你原来的完整重启提示逻辑
...
Filename: "{app}\VirtualBox-7.1.6-167084-Win.exe";StatusMsg: "Installing VirtualBox..."; Check: not IsVirtualBoxInstalled;AfterInstall: SetRestartNeeded; Flags: waituntilterminated;
