...
if %errorlevel% equ 0 (
    powershell -Command "$files = @(); $totalSize = 0; Get-ChildItem -Path '%folder%' -Recurse -File -ErrorAction SilentlyContinue | ForEach-Object { $files += @{ FullName = $_.FullName; Length = $_.Length }; $totalSize += $_.Length }; $result = @{ Files = $files; TotalSize = $totalSize }; ConvertTo-Json -InputObject $result -Depth 5 -Compress" 2>nul
    if %errorlevel% equ 0 (
        exit /b 0
    )
)
...
echo            files.push({ "FullName": file.Path.replace(/\\/g, "\\\\"), "Length": file.Size });
...
echo var result = { "Files": files, "TotalSize": totalSize };
echo WScript.Echo(JSON.stringify(result));
) > "%temp%\filesize.js"

cscript //nologo //E:JScript "%temp%\filesize.js" 2>nul
